...
export function registerChatHandlers(ctx: IpcContext): void {
  ipcMain.handle(IPC.CHAT_SEND, async (_event, input: SendMessageInput) => {
    return ctx.chatRuntime.sendMessage(input);
  });

  ipcMain.handle(IPC.CHAT_STREAM, async (_event, input: SendMessageInput) => {
  ...
  ipcMain.handle(IPC.CHAT_SET_HISTORY_LIMIT, (_event, limit: number) => {
    ctx.chatRuntime.chatHistoryLimit = limit;
  });

  ipcMain.handle(IPC.CHAT_GET_HISTORY_LIMIT, () => {
  ...
  ipcMain.handle(IPC.SESSIONS_LIST, async (_event, limit = 50) => {
    return ctx.chatRuntime.listSessions(limit);
  });

  ipcMain.handle(
    IPC.SESSIONS_MESSAGES,
  ...
  ipcMain.handle(IPC.SESSIONS_LIST_FILTERED, (_event, filter: SessionFilter, limit?: number) =>
    ctx.sessionRepo.listFiltered(filter, limit));

  ipcMain.handle(IPC.SESSIONS_UPDATE_TODO_STATE, (_event, sessionId: string, state: TodoStateKind) =>
    ctx.sessionRepo.updateTodoState(sessionId, state));

  ipcMain.handle(IPC.SESSIONS_TOGGLE_FLAG, (_event, sessionId: string) =>
    ctx.sessionRepo.toggleFlag(sessionId));

  ipcMain.handle(IPC.SESSIONS_TOGGLE_ARCHIVE, (_event, sessionId: string) =>
    ctx.sessionRepo.toggleArchive(sessionId));

  ipcMain.handle(IPC.SESSIONS_ADD_LABEL, (_event, sessionId: string, label: SapLabel) =>
    ctx.sessionRepo.addLabel(sessionId, label));

  ipcMain.handle(IPC.SESSIONS_REMOVE_LABEL, (_event, sessionId: string, label: SapLabel) =>
    ctx.sessionRepo.removeLabel(sessionId, label));

  ipcMain.handle(IPC.SESSIONS_STATS, () =>
