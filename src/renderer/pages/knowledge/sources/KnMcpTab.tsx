...
import { PlugZap, Unplug, Plus, RefreshCw, Loader2 } from 'lucide-react'
...
export function KnMcpTab({ domainPack }: KnMcpTabProps) {
  ...
  const mcpSources = configuredSources.filter((source) => source.kind === 'mcp')
          ...
          {connectedServers.map((serverName) => (
            <article key={serverName} className={`source-card ${selectedMcpServer === serverName ? 'active' : ''}`}>
              ...
              <div className="source-card-actions">
                <Button variant="secondary" size="sm" type="button" onClick={() => setSelectedMcpServer(serverName)}>
                  리소스 보기
                </Button>
