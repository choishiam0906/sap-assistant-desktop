...
에이전트는 여러 **스킬**을 순서대로 실행하는 자동화 워크플로우입니다. `*.agent.md` 파일을 작성하면 Enterprise Knowledge Hub에서 자동으로 로드되어 프리셋 에이전트와 함께 사용할 수 있습니다.
...
```
%APPDATA%/Enterprise Knowledge Hub/agents/
```
