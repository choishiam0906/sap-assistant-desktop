# Enterprise Knowledge Hub — 사용 가이드 (v8.0)

Enterprise Knowledge Hub는 로컬 우선(Local-First) 아키텍처로 민감한 기업 운영 데이터를 보호하면서, 다중 LLM과 커스텀 에이전트를 통해 운영 워크플로우를 자동화하는 플랫폼입니다.
...
Enterprise Knowledge Hub는 다음과 같은 작업을 지원합니다:

- **AI 기반 채팅**: OpenAI, Anthropic, Google 다중 LLM과 대화
- **코드/문서 영향 분석**: 커스터마이징 변경의 위험도 자동 분석
...
| 기능 | 설명 |
|------|------|
| **AI Chat** | 5가지 도메인 팩 선택 후 AI 채팅, 실시간 스트리밍 응답 |
| **Impact Analysis** | 코드 변경 텍스트 업로드 → 위험도/영향 분석 |
...
**Enterprise Knowledge Hub v8.0.0**
